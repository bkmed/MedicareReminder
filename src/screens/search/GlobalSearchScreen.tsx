...
export const GlobalSearchScreen = ({ navigation }: any) => {
    ...
    return {
      ...
      doctors: doctors.filter(
        d =>
          ...
          (d.specialty &&
            t(`specialties.${d.specialty}`).toLowerCase().includes(query)),
      ),
  ...
  const navigateToDetail = (type: string, item: any) => {
    if (Platform.OS === 'web' && setActiveTab) {
      ...
      setActiveTab(tabMap[type], screenMap[type], {
        [idMap[type]]: Number(item.id),
      });
    } else {
      const screenMap: any = {
        medication: 'MedicationDetails',
        ...
        {items.map(item => (
          ...
          >
            <Text style={styles.resultName}>
              {item.name || item.title || item.medicationName}
            </Text>
