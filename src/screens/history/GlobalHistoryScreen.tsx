...
export const GlobalHistoryScreen = () => {
  ...
  const loadHistory = async () => {
    try {
      ...
      const medicationMap = new Map(allMedications.map(m => [m.id, m.name]));

      const enrichedHistory: HistoryItem[] = allHistory
        .map(item => ({
          ...item,
          medicationName:
            medicationMap.get(item.medicationId) ||
            t('history.unknownMedication'),
        }))
        .sort(
          (a, b) =>
